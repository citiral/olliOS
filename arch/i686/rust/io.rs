...
pub unsafe fn outb(port: u8, value: u8)
{
	asm!("outb %al, %dx"::"{ax}" (value), "{dx}" (port) );
}

pub unsafe fn outw(port: u8, value: u16)
{
	asm!("outw %ax, %dx"::"{ax}" (value), "{dx}" (port) );
}

pub unsafe fn outl(port: u8, value: u32)
{
